...
e*e;return(2*b-2*c+a+d)*e*f+(-3*b+3*c-2*a-d)*f+a*e+b}}};"undefined"!=typeof module&&module.exports&&(module.exports=TWEEN);var UIL,loop,rootUpdate,NEO=NEO||function(){return{main:null,REVISION:"0.2",DEF:!1,DID:0,events:"onkeyup onkeydown onclick onchange onmouseover onmouseout onmousemove onmousedown onmouseup onmousewheel".split(" "),WIDTH:300,BW:190,AW:100,svgns:"http://www.w3.org/2000/svg",classDefine:function(){NEO.COLOR="NO";NEO.txt1='font-family:"Open Sans", sans-serif; font-size:11px; color:#cccccc; outline:0; padding:0px 10px; left:0; top:1px; height:17px; width:100px; overflow:hidden;';NEO.CC("NEO","position:absolute; pointer-events:none; box-sizing:border-box; -o-user-select:none; -ms-user-select:none; -khtml-user-select:none; -webkit-user-select:none; -moz-user-select:none; margin:0; padding:0; border:none; ");
NEO.CC("NEO.content","bottom:0; left:0; width:100px;  background:none; pointer-events:auto; transition:height, 0.1s ease-out;");NEO.CC("NEO.topcontent","bottom:0; left:0; width:100px; background:none;");NEO.CC("NEO.topmenu","width:100%; height:24px; background:none; ");NEO.CC("NEO.timeBar","width:100px; height:20px; top:20px; background:none; pointer-events:auto; cursor:e-resize;");NEO.CC("NEO.timescale","width:100px; height:20px; background:none; bottom:0; pointer-events:auto; cursor:e-resize;");
NEO.CC("NEO.inner","width:100%; top:40px; height:auto; overflow:hidden; background:none;");NEO.CC("NEO.base","position:relative; transition:height, 0.1s ease-out; height:80px; overflow:hidden;");NEO.CC("NEO.track","position:absolute; left:0; top:20px; width:100px; height:60px; overflow:hidden; pointer-events:auto; cursor:pointer; background:none; border-top:1px solid transparent; border-bottom:1px solid transparent;");NEO.CC("NEO.track:hover","border-top:1px solid #035fcf; border-bottom:1px solid #035fcf;");
...
a,b)}}}();
NEO.Timeline=function(a,b){this.data={};this.time=this.frame=0;this.height=60;this.width=100;this.maxTop=145;this.decal=b||0;this.frameSize=10;this.currentLeftFrame=0;this.viewFrame=Math.round(this.width/this.frameSize);this.fps=60;this.timerStep=1E3/this.fps;this.delta=this.now=0;this.then=Date.now();this.maxFrame=750;this.maxSize=this.frameSize*this.maxFrame;this.currentPosition=this.currentLeftFrame*this.frameSize;this.ratio=this.maxFrame/this.width;this.invRatio=this.width/this.maxFrame;this.viewFrame=
this.width/this.frameSize;this.posX=0;this.inPlay=this.scrolldown=this.timerdown=!1;this.neo=[];this.f=[];this.content=NEO.DOM("NEO content","div",a);document.body.appendChild(this.content);this.topcontent=NEO.DOM("NEO topcontent","div",a);document.body.appendChild(this.topcontent);this.top=parseFloat(this.content.style.top.substring(0,this.content.style.top.length-2));this.topmenu=NEO.DOM("NEO topmenu");this.topmenu.appendChild(this.liner(1));var c=function(a){this.scaletime(a)}.bind(this),d=function(a){this.setFps(a)}.bind(this),
...
"px";this.moveMarker();for(a=this.neo.length;a--;)this.neo[a].move()},moveMarker:function(){this.marker.style.left=(this.frame-this.currentLeftFrame)*this.frameSize+"px";this.miniFramePos.style.left=this.frame*this.invRatio+"px"},getFrameClick:function(a){a=Math.floor(a/this.frameSize)+this.currentLeftFrame;a>this.maxFrame&&(a=this.maxFrame);return a},scaletime:function(a){this.frameSize=Math.floor(10*a);this.maxSize=(this.maxFrame+1)*this.frameSize;a=[Math.round(this.frameSize)+0.5,Math.round(2*
this.frameSize)+0.5,Math.round(3*this.frameSize)+0.5,Math.round(4*this.frameSize)+0.5,Math.round(5*this.frameSize)+0.5];NEO.setSVG(this.patternLine,"d","M0.5 10 L0.5 18M"+a[0]+" 15 L"+a[0]+" 18M"+a[1]+" 15 L"+a[1]+" 18M"+a[2]+" 15 L"+a[2]+" 18M"+a[3]+" 15 L"+a[3]+" 18M 0 19.5 L"+a[4]+" 19.5",0);NEO.setSVG(this.pattern.childNodes[0],"width",5*this.frameSize,0);this.timeBar.style.width=this.maxSize+"px";for(a=this.neo.length;a--;)this.neo[a].setSize();a=0.5*this.frameSize+0.5;NEO.setSVG(this.marker,
"width",this.frameSize);NEO.setSVG(this.marker,"x1",a,1);NEO.setSVG(this.marker,"x2",a,1);this.setScaler();this.move()},setScaler:function(){this.ratio=this.maxFrame/this.width;this.viewFrame=Math.floor(this.width/this.frameSize);this.miniScaleView=Math.floor(this.invRatio*this.viewFrame);this.maxSize<this.width?this.scaler.style.display="none":(this.scaler.style.display="block",this.scaler.style.width=this.miniScaleView+"px",NEO.setSVG(this.scaler,"width",this.miniScaleView))},setFps:function(a){this.fps=
...
NEO.Color.prototype.setSize=function(){this.c[5].style.width=NEO.main.maxSize+"px";for(var a=NEO.main.frameSize,b=this.items.length,c;b--;)c=this.items[b],c.reSize(a);a=Math.floor(NEO.main.maxFrame/this.degNumber*NEO.main.frameSize);for(b=this.degrad.length;b--;)this.degrad[b].style.width=a+"px",this.degrad[b].style.left=a*b+"px"};
