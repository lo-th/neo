...
*/
    ...
    makePad: function ( model ) {
        ...
        let w = 200; 
        let d = (ww-w)*0.5, m = 20;
        Tools$1.dom( 'rect', '', { x: d, y: d,  width: w, height: w, fill:T.colors.back }, svg ); // 0
        Tools$1.dom( 'rect', '', { x: d+m*0.5, y: d+m*0.5, width: w - m , height: w - m, fill:T.colors.button }, svg ); // 1
        // Pointer
        Tools$1.dom( 'line', '', { x1: d+(m*0.5), y1: ww *0.5, x2: d+(w-m*0.5), y2: ww * 0.5, stroke:T.colors.back, 'stroke-width': 2 }, svg ); // 2
        Tools$1.dom( 'line', '', { x1: ww * 0.5, x2: ww * 0.5, y1: d+(m*0.5), y2: d+(w-m*0.5), stroke:T.colors.back, 'stroke-width': 2 }, svg ); // 3
        Tools$1.dom( 'circle', '', { cx: ww * 0.5, cy: ww * 0.5, r:5, stroke: T.colors.text, 'stroke-width': 5, fill:'none' }, svg ); // 4
        T.pad2d = svg;
...
class Proto {
    ...
    constructor( o = {} ) {
        ...
        this.transition = o.transition || Tools$1.transition;
...
class Color$1 extends Proto {
    ...
    constructor( o = {} ) {
	    ...
	    this.value = '#ffffff';
	    if( o.value !== undefined ){
	        if( o.value instanceof Array ) this.value = Tools$1.rgbToHex( o.value );
	        else if(!isNaN(o.value)) this.value = Tools$1.hexToHtml( o.value );
	        else this.value = o.value;
	    }
	...
	mousemove ( e ) {
	    ...
	    let off, d, hue, sat, lum, rad, x, y, rr, T = Tools$1;
	...
	setValue ( v ){
		...
		if( v instanceof Array ) this.value = Tools$1.rgbToHex( v );
        else if(!isNaN(v)) this.value = Tools$1.hexToHtml( v );
        else this.value = v;
...
class Knob extends Proto {
    ...
    constructor( o = {} ) {
        ...
        if ( this.model > 0 ) {
            ...
            Tools$1.dom( 'path', '', { d: '', stroke:cc.text, 'stroke-width': 2, fill: 'none', 'stroke-linecap': 'round' }, this.c[3] ); //4
...
class Pad2D extends Proto {
    ...
    mousemove ( e ) {
        ...
        y = Tools$1.clamp( y, -this.maxPos, this.maxPos );
...
export { Map, Pannel, REVISION, Timeline, Tools, Utils, add };
